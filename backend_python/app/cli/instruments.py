...
@app.command()
def fetch(
    stocks: bool = typer.Option(True, "--stocks/--no-stocks", help="Fetch NSE equities"),
    mf: bool = typer.Option(True, "--mf/--no-mf", help="Fetch AMFI mutual funds"),
):
    """Fetch and refresh instruments from public sources (NSE + AMFI)."""
    from app.services.instrument_fetch_service import fetch_amfi_mutual_funds, fetch_nse_stocks, reset_log

    reset_log()
    db = SessionLocal()
    try:
        if stocks:
            typer.echo("Fetching NSE equities...")
            added, updated, skipped = fetch_nse_stocks(db)
            typer.echo(f"  → added {added}, updated {updated}, skipped {skipped}")

        if mf:
            typer.echo("Fetching AMFI mutual funds (direct growth only)...")
            added, updated, skipped = fetch_amfi_mutual_funds(db)
            typer.echo(f"  → added {added}, updated {updated}, skipped {skipped}")
