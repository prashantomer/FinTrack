...
_INSTRUMENT_REQUIRED = {InvestmentType.stock, InvestmentType.mutual_fund}


def _validate(data: InvestmentCreate | InvestmentUpdate, type_: InvestmentType | None = None) -> None:
    t = type_ if isinstance(data, InvestmentUpdate) else data.type
    if t in _INSTRUMENT_REQUIRED and not data.user_instrument_id:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"{t.value} investments must link to a tracked instrument (user_instrument_id required)",
        )
