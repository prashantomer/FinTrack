import { AtSign, Bot, Download, FileSpreadsheet, Pin, PinOff, User as UserIcon, Wrench } from 'lucide-react'
...
function ToolFileCard({ message }: { message: AssistantMessage }) {
  if (!message.file_url || !message.file_name) return null
  const rowCount = (message.tool_result as Record<string, unknown> | null)?.row_count
  const targetImport = (message.tool_result as Record<string, unknown> | null)?.target_import_type
  return (
    <a
      href={message.file_url}
      download={message.file_name}
      className="flex items-center gap-3 px-3 py-2.5 rounded-lg border bg-background hover:bg-muted/40 transition-colors group"
    >
      <span className="shrink-0 size-9 rounded-md bg-primary/10 text-primary flex items-center justify-center">
      ...
      <span className="flex-1 min-w-0 flex flex-col gap-0.5">
        ...
        <span className="text-[11px] text-muted-foreground flex items-center gap-2">
          ...
          {typeof targetImport === 'string' && (
            <>
              <span>·</span>
              <span>{targetImport.replace('_', ' ')} import</span>
            </>
          )}
          <span>·</span>
          <span>generated by {message.tool_name}</span>
        </span>
      </span>
      <span className="shrink-0 size-8 rounded-md flex items-center justify-center text-muted-foreground group-hover:text-foreground">
        <Download size={14} />
      </span>
    </a>
  )
...
function ToolDetailsRow({ message }: { message: AssistantMessage }) {
  return (
...
function ToolBubble({ message }: { message: AssistantMessage }) {
  return (
    <div className="self-start max-w-[85%] flex flex-col gap-2">
      <ToolFileCard message={message} />
      <ToolDetailsRow message={message} />
    </div>
  )
}

export function ToolGroupBubble({ messages }: { messages: AssistantMessage[] }) {
  if (messages.length === 1) return <ToolBubble message={messages[0]} />

  const uniqueNames = Array.from(new Set(messages.map(m => m.tool_name).filter(Boolean)))
  const summaryLabel =
    uniqueNames.length === 1
      ? <>{messages.length} × <code className="font-mono">{uniqueNames[0]}</code></>
      : <>{messages.length} tool calls</>

  return (
    <div className="self-start max-w-[85%] flex flex-col gap-2">
      {messages.map(m => <ToolFileCard key={`f${m.id}`} message={m} />)}
      <details className="rounded-md border border-dashed bg-muted/30 px-3 py-2 text-xs">
        <summary className="cursor-pointer flex items-center gap-1.5 text-muted-foreground">
          <Wrench size={11} /> tool · {summaryLabel}
        </summary>
        <div className="mt-2 flex flex-col gap-1.5">
          {messages.map(m => <ToolDetailsRow key={m.id} message={m} />)}
        </div>
      </details>
    </div>
  )
