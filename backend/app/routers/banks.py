...
@router.get("/api/v1/banks", response_model=list[BankRead])
def get_banks(
...
@router.post("/api/v1/accounts", response_model=AccountRead, status_code=status.HTTP_201_CREATED)
def create_acct(
    body: AccountCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.put("/api/v1/accounts/{account_id}", response_model=AccountRead)
def update_acct(
    account_id: int,
    body: AccountUpdate,
    db: Session = Depends(get_db),
...
@router.post("/api/v1/accounts/{account_id}/close", response_model=AccountRead)
def close_acct(
    account_id: int,
    body: AccountClose,
    db: Session = Depends(get_db),
...
@router.post("/api/v1/accounts/{account_id}/adjust", response_model=AccountRead)
def adjust_acct(
    account_id: int,
    body: BalanceAdjust,
    db: Session = Depends(get_db),
